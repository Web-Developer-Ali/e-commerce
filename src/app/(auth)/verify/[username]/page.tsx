...
function Page() {
  ...
  const encodedParams = params.username;
  ...
  // Access the username and respass values
  const username = getUrlData['username'];
  const respass = getUrlData['respass'];
  ...
  const onSubmit: SubmitHandler<z.infer<typeof verifySchema>> = async (data) => {
    ...
    try {
      const responce = await axios.post('/api/verify-code', {
        username: username,
        code: data.code, // Use data.code instead of otp
      });
      toast({
        title: "Success",
        description: responce.data.message,
      });
     // Navigate to reset password page if respass is true
     if (respass === 'true') {
      route.replace('/change_user_password');
    } else {
  ...
  return (
    <section className="dark:bg-gray-800 min-h-screen bg-gray-50 flex items-center justify-center">
      <div className="w-full max-w-md bg-white rounded-lg shadow p-6">
        <h1 className="text-2xl text-black font-bold mb-4">Verify OTP</h1>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div>
            ...
            <input
              ...
              className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 text-black dark:taxt-white sm:text-sm"
              required
          ...
          <button
            type="submit"
            className="w-full bg-blue-500 text-white py-2 px-4 rounded-lg focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
